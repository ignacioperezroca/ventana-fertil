...
export function GenericShareCard({
  ...
  const handleShare = async () => {
    try {
      trackEvent("share_app_clicked", { source: "app_card", hasResult: false, isDemo, eventVersion: "g1" });
      const usedNative = await shareApp();
      if (!usedNative) {
        await copyShareUrl();
        onNotify("success", "Link copiado", "No había hoja nativa, así que copiamos el enlace.");
        return;
      }
  ...
  const handleCopyLink = async () => {
    try {
      trackEvent("share_app_clicked", { source: "copy_link", hasResult: false, isDemo, eventVersion: "g1" });
      await copyShareUrl();
    ...
    <section className="rounded-[34px] border border-app-border bg-[linear-gradient(180deg,rgba(255,255,255,0.94)_0%,rgba(255,249,244,0.92)_100%)] p-4 shadow-[0_24px_70px_-46px_rgba(36,22,47,0.34)] sm:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-app-muted">Este card nunca incluye fechas íntimas ni notas del ciclo.</p>
        </div>
