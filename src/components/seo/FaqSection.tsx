...
export function FaqSection({ isDemo = false }: { isDemo?: boolean }) {
    ...
    <section className="rounded-[34px] border border-app-border bg-white/92 p-4 shadow-[0_24px_70px_-46px_rgba(36,22,47,0.34)] sm:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-app-muted">Respuestas cortas para entender la estimación sin leer bloques largos.</p>
        </div>
          ...
          <details
            ...
            onToggle={(event) => {
              if ((event.currentTarget as HTMLDetailsElement).open) {
                trackEvent("faq_opened", { source: `faq-${index + 1}`, hasResult: false, isDemo, eventVersion: "g1" });
              }
            }}
          >
